...
### Setting variables defining input/output files
  ...
  delimiter <- ("|")
  outputName <- ("MyDataAnalysis_01.txt")
...
# 
#   AAC <- matrix(NA, nrow = attrCount, ncol=5)
#   for (x in 1:2){
...
#   }
#   #print(AAC)
#   write.table(x=AAC, file=configFile, sep = ",")
# }
...
cat(sprintf("rogram will step throught each attribute and allow selection of \n"))
...
#     isNumber <- sum(!is.na(as.integer(trim(colList))))
